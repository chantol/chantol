<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click','.btnsavepurchase',function(e){
				...
				if(tbrow==0){
					...
					var frmdata=$('#frmpurchase').serialize();
					var url="{{ route('storepurchase') }}"
		...
		$(document).on('click','.btnupdate-purchase',function(e){
			e.preventDefault();
			var tbrow = document.getElementById("invoice_items").rows.length-4;
			if(tbrow==0){
				alert('No Row Update Found !')
			}else{
				var frmdata=$('#frmeditpurchase').serialize();
				var url="{{ route('updatepurchase') }}"
				$.post(url,frmdata,function(data){
					console.log(data)
					if($.isEmptyObject(data.error)){
        ...
        $(document).on('click','.rowedit',function(e){
        	...
        	if(submit==1){
	        	...
	        	if(c==true){
	        		var url="{{ route('edititemaftersubmitin') }}";
		...
		$(document).on('click','.btnedit',function(e){
			...
			url="{{ route('editpurchaseinv') }}";
			$.get(url,{id:invid},function(data){
				//console.log(data)
				$('#frmtitle').text('Purchase Update');
				$('#frmpurchase').attr('action',"{{ route('updatepurchase') }}");
				$('#orderid').text('Inv#' + invid_str )
				$('#purchase_id').val(invid);
	...
	'<input type="text" style="width:100%;height:40px;font-size:18px;border-style:none;text-align:right;padding:8px;" id="subtotal" name="subtotal" value="'+ subtotal +'">'+
							...
							'</tr>'+
	    							'<tr>'+
	    								'<td colspan="12" style="border:none;padding:0px;"><input type="text" class="form-control" name="invnote" style="font-family:Khmer Os System;width:99%;height:40px;padding:8px;margin-left:5px;" value="'+ invdesr +'">'+
	    								'</td>'+
	    								...
	    								'<td colspan="12" style="border:none;">'+
	    									'<div class="col-lg-6">'+
	    										...
	    										'<div class="input-group input-group-sm" style="width: 300px;">'+
								                  '<input type="text" name="barcode_search" class="form-control pull-right barcode_search" placeholder="Search Barcode">'+
	    									...
	    									'</div>'+
	    										'<div class="col-lg-6">'+
	    											'<button class="btn btn-warning btn-sm pull-right btnupdatepurchase" style="color:blue;font-family:khmer os system;">កែប្រែ</button>'+
	    											'<button class="btn btn-default btn-sm pull-right btnnew" style="color:blue;font-family:khmer os system;margin-right:10px;">វិក័យប័ត្រថ្មី</button>'+
	    										'</div>'+
		...
		function readinvdetail(nn,pid,barcode,name,qty1,unit,qty2,qty3,unitprice,discount,amount,cur,multi,qtyunit,submit,focunit,itemunit) {
			var newrow='<tr>' +
						...
						'</td>'+
						'<td style="padding:0px;width:50px">'+
						'<input type="text" class="form-control cur1" name="cur1[]" required style="border-style:none;" value="'+ cur +'">'+
						'</td>'+
						'<td style="padding:0px;width:80px">'+
						...
						'</td>'+
						'<td style="padding:0px;width:50px">'+
						'<input type="text" class="form-control multi" name="multi[]" required style="border-style:none;"​ readonly value="'+ multi +'">'+
						'</td>'+
						// '<td style="padding:0px;width:100px">'+
						// '<input type="text" class="form-control qtyunit" name="qtyunit[]" required style="border-style:none;width:100px;" readonly value="'+ qtyunit +'">'+
						// '</td>'+
						'<td style="padding:0px;width:80px">'+
						...
						'<td style="text-align:center;padding:0px;width:100px;">'+
							...
							'<a href="#" class="btn btn-danger btn-xs rowremove" style="margin-top:5px;" data-submit="'+ submit +'"><i class="fa fa-minus"></i></a>'+
						'</td>'+
						'<td style="padding:7px;width:100px;visibility:hidden;">'+
						'<input type="text" class="form-control " name="productid[]" required style="border-style:none;width:100px;display:none;" value="'+ pid +'">'+
						'</td>'+
					'</tr>';
		        ...
		        $.ajax({
		          ...
		          success:function(data){
		          	if(data){
						...
						'<td style="padding:0px;width:70px;">'+
						'<input type="text" class="form-control qty1 canenter" name="qty1[]" required style="border-style:none;padding:0px;text-align:center;width:70px;" value="'+ qty +'">'+
						'</td>'+
						'<td style="padding:0px;width:50px;">'+
						...
						'<td style="padding:0px;width:70px;">'+
						'<input type="text" class="form-control qty3" name="qty3[]" required style="border-style:none;padding:0px;text-align:center;width:70px;" value="'+ qty +'">'+
						'</td>'+
						'<td style="padding:0px;width:50px;">'+
						...
						'<td style="padding:0px;width:120px;">'+
						'<input type="text" class="form-control text-right unitprice canenter allownumericwithdecimal" name="unitprice[]" required '+'style="border-style:none;width:120px;" value="'+ pri +'">'+
						'</td>'+
						'<td style="padding:0px;width:40px;">'+
							'<select name="cur[]" class="form-control cur canenter" style="border-style:none;margin-top:0px;padding:0px;width:40px;">'+
						...
						'</td>'+
						'<td style="padding:0px;text-align:right; width:270px;" colspan=2>'+
						'<input type="text" class="form-control text-right amount" name="amount[]" required style="border-style:none;font-weight:bold;width:270px;" value="'+ formatNumber(parseFloat(qty) * parseFloat(pri)) +'">'+
						'</td>'+
						'<td style="padding:0px;width:50px">'+
						'<input type="text" class="form-control cur1" name="cur1[]" required style="border-style:none;width:50px;" value="R">'+
						'</td>'+
						'<td style="padding:0px;width:80px">'+
						...
						'</td>'+
						'<td style="padding:0px;width:60px">'+
						'<input type="text" class="form-control multi" name="multi[]" required style="border-style:none;width:60px;"​ readonly value="'+ data.multiple +'">'+
						'</td>'+
						// '<td style="padding:0px;width:80px">'+
						...
						'<td style="text-align:center;padding:0px;;width:100px;">'+
							'<a href="#" class="btn btn-warning btn-xs rowedit" style="color:blue;margin-top:5px;"><i class="fa fa-pencil"></i></a>'+
							'<a href="#" class="btn btn-danger btn-xs rowremove" style="margin-top:5px;"><i class="fa fa-minus"></i></a>'+
						'</td>'+
						'<td style="padding:7px;width:100px;visibility:hidden;">'+
						'<input type="text" class="form-control " name="productid[]" required style="border-style:none;width:100px;display:none;" value="'+ data.product_id +'">'+
						'</td>'+
					'</tr>';
		          	...
		          	{
		          		alert('This barcode can not be found.');
		          	}
		            console.log(data);
...
//-----------------------
      ...
      $(document).on('click','.btndelinv',function(e){
        e.preventDefault();
        inv_num=$(this).data('id');//or =$(this).attr('id');

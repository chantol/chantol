...
@section('content')
	<span id="success_message"></span>
	 <div class="panel panel-default">
      <div class="panel-heading"><b>*** Stock Out ***</b></div>
      <form method="POST" id="frmsalesubmit" action="#">
	   {{-- {{ csrf_field() }} --}}
			    	...
			    	<div class="col-md-12">
			    		<div class="panel panel-default">
			    			<div class="panel-heading">
			    				...
			    				<div class="col-sm-6">
			    					<button class="btn btn-primary btn-sm pull-right" style="margin-top:1px;font-weight:bold;"  id="btnsubmit">Submit all</button>
			    				</div>
			    			</div>
			    			<!-- Image loader -->
							<div id="fade"></div>
...
//-------------------------------------------
			...
			$(document).on('click','.btnsubmitbyitem',function(e){
				...
				var dd=currentRow.find("td:eq(1) input").val();
         		...
         		var exchangeamount=currentRow.find("td:eq(14) input").val(); 
         		var pcur=currentRow.find("td:eq(15) input").val(); 
         		var submit=currentRow.find("td:eq(16) input").val(); 
         		var userid=$('#userid').val();
					var url="{{ route('submitbyitem') }}";
					$.post(url,{userid:userid,dd:dd,pid:pid,qty:qty,foc:foc,unit:unit,amount:amount,cur:cur,submit:submit,amountusd:exchangeamount,pcur:pcur},function(data){
						console.log(data)
	     				if(data.savesuccess){
	     					//alert('save success')
			...
			$(document).on('click','#btn_yes1',function(e){
				...
				var dd=currentRow.find("td:eq(1)").text();
         		...
         		var cur=currentRow.find("td:eq(13) input").val(); 
         		var userid=$('#userid').val();
				var url="{{ route('delandsubmitbyitem') }}";
			...
			$(document).on('click','#btnsubmit',function(e){
				...
				if(c==true){
					var tbrow = document.getElementById("summary_salelist").rows.length;
				if(tbrow>0){
					$.ajax({
						method:"POST",
						url:"{{ route('submititemsale') }}",
						data:$('#frmsalesubmit').serialize(),

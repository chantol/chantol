...
@section('content')
	 <div class="panel panel-default">
      <div class="panel-heading"><b>*** Sale Invoice ***</b></div>
      		...
      		<div class="row">
					<div class="col-lg-12">
						<div class="panel panel-body">
							...
							<div class="col-lg-3" id="fillsearch">
	    	...
	    	<div class="col-md-12">
	    		<div class="panel panel-default">
	    			<div class="panel-heading">
	    				<h3 class="panel-title"><strong>Invoice Sale List</strong></h3>
	    			</div>
	    			<div class="panel-body" id="table_data">
	    				 @include('sales.salelist')
	    			</div>
...
//-------------------------------------------
			$(document).on('change','#typesearch',function(e){
				...
				if(sel_val==5){
					 el='<label for="select">Search By Law:</label>'+
					'<div class="input-group">'+
					...
					'@endforeach'+
					'</select>'+
					'<div class="input-group-addon btn btn-default" id="search_law">'+
						'<span class="fa fa-search" ></span>'+
					'</div></div>';
				...
				if(sel_val==6){
					 el='<label for="select">Search By Invoice#:</label>'+
					'<div class="input-group">'+
			...
			$(document).on('click','#search_barcode',function(e){
				...
				var url="{{ route('invoicelistsearchinsale') }}";
				$.get(url,{search_by:'barcode',barcode:barcode,start_date:start_date,end_date:end_date},function(data){
					console.log(data)
			...
			$(document).on('click','#search_productcode',function(e){
				...
				var url="{{ route('invoicelistsearchinsale') }}";
				$.get(url,{search_by:'productcode',productcode:productcode,start_date:start_date,end_date:end_date},function(data){
			...
			$(document).on('click','#search_invoice',function(e){
				...
				var start_date=$('#start_date').val();
				var end_date=$('#end_date').val();
				$('#search_by').val('invoice');
				$('#search_val').val(invoice);
				var url="{{ route('invoicelistsearchinsale') }}";
				$.get(url,{search_by:'invoice',invoice:invoice,start_date:start_date,end_date:end_date},function(data){
			...
			$(document).on('click','#search_law',function(e){
				var law=$('#law').val();
				var start_date=$('#start_date').val();
				var end_date=$('#end_date').val();
				$('#search_by').val('law');
				$('#search_val').val(law);
				var url="{{ route('invoicelistsearchinsale') }}";
				$.get(url,{search_by:'law',law:law,start_date:start_date,end_date:end_date},function(data){
			...
			$(document).on('click','#search_delivery',function(e){
				...
				var url="{{ route('invoicelistsearchinsale') }}";
				$.get(url,{search_by:'delivery',delivery:delivery,start_date:start_date,end_date:end_date},function(data){
			...
			$(document).on('click','#search_supplier',function(e){
				...
				var url="{{ route('invoicelistsearchinsale') }}";
				$.get(url,{search_by:'supplier',supplier:supplier,start_date:start_date,end_date:end_date},function(data){
			...
			$(document).on('click','.btnsearchbydate',function(e){
				...
				$.get(url,{search_by:'date',start_date:start_date,end_date:end_date},function(data){
			...
			function fetch_data(page,start_date,end_date,search_by,search_val) {
				$.ajax({
						...
						url:"/sale/pagination/fetch_data?page="+ page +"&start_date="+ start_date +"&end_date="+ end_date +"&search_by="+ search_by +"&search_val="+search_val,
						success:function(data)
						{
							//console.log(data)
							$('#table_data').empty().html(data);

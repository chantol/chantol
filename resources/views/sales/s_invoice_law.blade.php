	    								...
	    								<tr>
											...
											<td style="text-align:center;">
												<div class="custom-control custom-checkbox">
								                  <input type="checkbox" class="custom-control-input customCheck" style="height:20px;width:20px;margin-top:0px;" {{ $inv->deposit_lawfee>=$inv->lawfee ?'disabled':'' }}>
								              	</div>
	    									...
	    									<td style="text-align:center;text-decoration: underline;">
	    										<a href="#" data-id="{{ $inv->id }}" data-deposit="{{ $inv->deposit_lawfee }}" data-cur="{{ $inv->cur }}" data-supname="{{ $inv->law->name }}" data-totalinv="{{ $inv->lawfee }}" class="showpaiddetail">{{ phpformatnumber($inv->deposit_lawfee) }}</a>
	    									</td>
	    									<td style="text-align:center;">{{ $inv->cur }}</td>
	    									<td style="text-align:center;">{{ phpformatnumber((float)$inv->lawfee-(float)$inv->deposit_lawfee) }}</td>
	    									<td style="text-align:center;">{{ $inv->cur }}</td>

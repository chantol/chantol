...
@section('content')
	 ...
	 <div class="panel panel-default">
      <div class="panel-heading" style="background:blue;color:white;"><b>***Delivery Payment</b></div>
			    	...
			    	<div class="col-md-12">
			    		<div class="panel panel-default">
			    			<div class="panel-heading">
			    				<h3 class="panel-title" style="display:inline;"><strong>Delivery Invoice List</strong></h3>
			    			...
			    			<div class="panel-body" id="table_data">
			    				@include('sales.s_invoice_delivery')
			    			</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$(document).on('click','.showpaiddetail',function(e){
				...
				$.get(url,{id:invid,delivery:'delivery'},function(data){
					//console.log(data)
			...
			$(document).on('click','.btndelpaid',function(e){
				...
				if(c==true){
					...
					$.post(url,{id:paid_id,purinv:purinv,totalinv:totalinv,paymethod:'delivery'},function(data){
						console.log(data)
						//alert(data[0].tpaid)
						$('#ppdt').empty().html(data);
						$('#tpaid').val(formatNumber($('#totalpaid').val()) + cur);
...
//-------------------------------------------
			...
			$(document).on('click','#search_supplier',function(e){
				...
				var url="{{ route('invoicelistsearchpaidfordelivery') }}";
			...
			$(document).on('click','#btnpaidnow',function() {
            	...
            	$('#paymethod').val('delivery');
	            $("#invoicelist input[type=checkbox]:checked").each(function (){
	                var row = $(this).closest("tr");
	                if(i==0){
	                	 customer0=row.find("td:eq(7)").text();
	                	 cur0=row.find("td:eq(14)").text();
	                }
	                var customer1=row.find("td:eq(7)").text();
	                cur=row.find("td:eq(14)").text();

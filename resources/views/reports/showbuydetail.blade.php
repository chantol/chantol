		...
		<div class="row">
			<div class="panel panel-default">
				<div class="panel-heading" style="font-family:khmer os system;font-size:18px;">
					<div class="col-lg-8">
						របាយការណ៌ទិញផលិផល:  {{ $saledetails[0]->product->name }}  សំរាប់ថ្ងៃទី: {{ date('d-m-Y',strtotime($saledetails[0]->purchase->invdate)) }}
						<p style="color:blue;">*អ្នកផ្គត់ផ្គង់ {{ $supname }}</p>
					</div>
					<div class="col-lg-4">
						...
						<tbody>
							@foreach ($saledetails as $key => $sd)
								<tr>
									...
									<td class="text-center">{{ date('d-m-Y',strtotime($sd->purchase->invdate)) }}</td>
									<td class="text-center">{{ sprintf("%04d",$sd->purchase_id) }}</td>
									<td class="text-center">{{ $sd->purchase->supplier->name }}</td>
